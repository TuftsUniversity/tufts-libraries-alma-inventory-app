...
$(document).ready(function(){
  ...
  // safe no-op if dropdowns not present
  // if ($('#libSelected').length) {
  //  populateLibs();
  //} else {
  //  $(document).on('DOMNodeInserted', '#libSelected', populateLibs);
  //}
    // retry until #libSelected exists
  let waitForLib = setInterval(function() {
    if ($('#libSelected').length) {
...
function parseResponse(barcode, json) {
  ...
  if ('errorsExist' in json) {
    ...
    var tempLoc = (holdingData.temp_location && holdingData.temp_location.desc && holdingData.in_temp_location == true) 
                ? holdingData.temp_location.desc 
                : "";
  ...
  $.ajax({
    ...
    success: function(json, textStatus, jqXHR) {
      ...
      if ('errorsExist' in json) {
        ...
        $('#libSelected').on('change', function() { populateLocs(); } );    
      }
    },
